...
export default function CustomerScreen() {
  ...
  // Fetch customers for signed-in user
  useEffect(() => {
    if (!currentUser) {
      setCustomers([]);
    ...
    const unsub = onSnapshot(q as any, (snapshot: QuerySnapshot<DocumentData>) => {
      const fetched: Customer[] = snapshot.docs.map((docSnap: QueryDocumentSnapshot<DocumentData>) => ({
        id: docSnap.id,
        ...(docSnap.data() as any),
      })) as Customer[];
  ...
  const validate = () => {
    ...
    if (!trimmedName) newErrors.name = 'Customer name is required.';
    const duplicate = customers.some(customer =>
      customer.name.trim().toLowerCase() === trimmedName.toLowerCase() && customer.id !== editingId
    );
    if (duplicate) newErrors.name = 'Customer name already exists.';
  ...
  const openEditModal = (customer: Customer) => {
    ...
    setContactNumbers(customer.contactNumbers.length ? customer.contactNumbers : ['']);
  ...
  const handleAddOrUpdate = async () => {
    ...
    setLoading(true);
    if (!currentUser) {
      Alert.alert('Sign in required', 'Please sign in to manage customers.');
      setLoading(false);
      return;
    }
    ...
    if (editingId) {
      try {
        ...
        if (!data || data.ownerId !== currentUser.uid) {
          Alert.alert('Permission denied', 'You can only edit customers you created.');
          setLoading(false);
          return;
        }

        await updateDoc(customerRef, { name, contactNumbers, balance });
      } catch (error) {
      ...
      try {
        ...
        Alert.alert('Error', 'Failed to add customer to Firestore.');
        setLoading(false);
        return;
      }
    }
    ...
    Alert.alert('Delete Customer', 'Are you sure you want to delete this customer?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
          ...
          try {
            if (!currentUser) {
              Alert.alert('Sign in required', 'Please sign in to delete customers.');
              setLoading(false);
              return;
            }
            ...
            const data = snap.exists() ? (snap.data() as any) : null;
            if (!data || data.ownerId !== currentUser.uid) {
              Alert.alert('Permission denied', 'You can only delete customers you created.');
              setLoading(false);
              return;
            }
      ...
      >
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerTitles}>
            <ThemedText type="title" style={styles.title}>Customer Management</ThemedText>
            <ThemedText type="subtitle" style={styles.subtitle}>Manage your business customers and clients</ThemedText>
          </View>
        </View>
      </LinearGradient>
          ...
          <View style={styles.contentContainer}>
            ...
            >
              <View style={styles.customersHeader}>
                ...
                <View style={styles.customersHeaderText}>
                  <ThemedText type="title" style={styles.customersTitle}>All Customers</ThemedText>
                  <ThemedText style={styles.customersSubtitle}>
                    {filteredCustomers.length} customers found
                  </ThemedText>
                </View>
                <Pressable style={styles.addBtn} onPress={openAddModal}>
                  <MaterialIcons name="add" size={20} color="#fff" />
                  <ThemedText style={styles.addBtnText}>Add Customer</ThemedText>
                </Pressable>
              </View>
              ...
              {initialLoading ? (
                ...
                <View style={styles.customersList}>
                  {customers.filter(customer => customer.name.toLowerCase().includes(search.trim().toLowerCase())).length === 0 ? (
                          ...
                          <View style={styles.customerHeader}>
                            ...
                            <View style={styles.customerInfo}>
                              ...
                              <Text style={styles.customerContact}>
                                ...
                                {' '}{item.contactNumbers && item.contactNumbers.length > 0 ? item.contactNumbers.join(', ') : 'No contact'}
                              </Text>
                              <View style={styles.balanceContainer}>
                                <MaterialIcons name="account-balance-wallet" size={14} color="#64748b" />
                                <Text style={[
                                  styles.balanceText, 
                                  Number(item.balance) < 0 ? styles.negativeBalance : styles.positiveBalance
                                ]}>
                                  {' '}Balance: Rs {Number(item.balance).toLocaleString()}
                                </Text>
                              </View>
                            </View>
            ...
            <View style={styles.modalHeader}>
              <View style={styles.modalHeaderLeft}>
                <View style={styles.modalIcon}>
                  <MaterialIcons name={editingId ? "edit" : "people"} size={24} color="#fff" />
                </View>
                <View>
              ...
              <View style={styles.formCard}>
                <View style={styles.formSection}>
                  <ThemedText style={styles.fieldLabel}>
                    <MaterialIcons name="person" size={16} color="#667eea" /> Customer Name *
                  </ThemedText>
                  <TextInput
                    style={styles.input}
