...
export default function SalesScreen() {
  ...
  const [filteredCustomerSuggestions, setFilteredCustomerSuggestions] = useState<CustomerSuggestion[]>([]);
  const [showItemSuggestions, setShowItemSuggestions] = useState(false);
  const [showCustomerSuggestions, setShowCustomerSuggestions] = useState(false);
  const customerInputRef = useRef<TextInput>(null);

  // Fetch items and customers for autocomplete
  // Subscribe to auth state to know the signed-in user
  useEffect(() => {
  ...
  // Fetch items and customers for autocomplete, scoped to current user when possible
  useEffect(() => {
    // If we have a signed in user, try to query only their docs (expects an `ownerId` field on docs).
    const itemsRef = currentUser ? query(collection(db, 'inventoryItems'), where('ownerId', '==', currentUser.uid)) : collection(db, 'inventoryItems');
    const customersRef = currentUser ? query(collection(db, 'customers'), where('ownerId', '==', currentUser.uid)) : collection(db, 'customers');
    ...
    const unsubItems = onSnapshot(itemsRef as any, (snapshot: QuerySnapshot<DocumentData>) => {
      const fetched: ItemSuggestion[] = snapshot.docs.map((docSnap: QueryDocumentSnapshot<DocumentData>) => {
        ...
        return {
          id: docSnap.id,
          name: data.name,
          sellingPrice: String(data.salesPrice ?? data.purchasingPrice ?? ''),
        };
      });
      setItemSuggestions(fetched);
    });

    const unsubCustomers = onSnapshot(customersRef as any, (snapshot: QuerySnapshot<DocumentData>) => {
      const fetched: CustomerSuggestion[] = snapshot.docs.map((docSnap: QueryDocumentSnapshot<DocumentData>) => {
        const data = docSnap.data();
        return {
          id: docSnap.id,
          name: data.name,
      ...
      setCustomerSuggestions(fetched);
    });
  ...
  const [customerPrevBalance, setCustomerPrevBalance] = useState<number>(0);
  const [cashReceived, setCashReceived] = useState('');
  const [onAccount, setOnAccount] = useState('');
  const [errors, setErrors] = useState<any>({});
  const [loading, setLoading] = useState(false);

  // Add or update item in sale list
    ...
    if (field === 'name') {
      if (value.trim().length > 0) {
        setFilteredCustomerSuggestions(
          customerSuggestions.filter(v => v.name.toLowerCase().includes(value.trim().toLowerCase()))
        );
  ...
  const handleCustomerSuggestionSelect = (suggestion: CustomerSuggestion) => {
    setCustomer({ name: suggestion.name, phone: suggestion.contactNumbers[0] || '' });
    if (typeof suggestion.balance === 'number') {
      setCustomerPrevBalance(suggestion.balance);
    } else if (typeof suggestion.balance === 'string' && !isNaN(Number(suggestion.balance))) {
      setCustomerPrevBalance(Number(suggestion.balance));
    } else {
  ...
  setItemPrice(String(suggestion.sellingPrice ?? ''));
    setShowItemSuggestions(false);
  };
  ...
  const handleSubmitSale = async () => {
    const newErrors: any = {};
    if (!customer.name.trim()) newErrors.customerName = 'Customer name required';
    if (!items.length) newErrors.items = 'Add at least one item';
    setErrors(newErrors);
    if (Object.keys(newErrors).length > 0) return;

    // Must be signed in to save a sale (multi-user support)
    if (!currentUser) {
      Alert.alert('Sign in required', 'Please sign in to save sales.');
      return;
    }

    setLoading(true);
    try {
      // 1. Save sale detail with owner info
      const saleRef = await addDoc(collection(db, 'sales'), {
        ...
        totalAmount,
        cashReceived: cash,
        onAccount: customerPrevBalance + totalAmount - cash,
        date: new Date().toISOString(),
      });

      // 2. Update customer balance (increase by net owed) only for the matched customer doc
      const customerDoc = customerSuggestions.find(v => v.name === customer.name);
      if (customerDoc) {
        const customerDocRef = doc(db, 'customers', customerDoc.id);
        await updateDoc(customerDocRef, {
          balance: (customerPrevBalance + totalAmount - cash),
        });
      }

      Alert.alert('Sale Saved', 'Your sale has been recorded successfully.');
      setItems([]);
      ...
      >
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerTitles}>
            <ThemedText type="title" style={styles.title}>New Sale</ThemedText>
            <ThemedText type="subtitle" style={styles.subtitle}>Record customer transactions and payments</ThemedText>
            {currentUser && (
              <ThemedText style={[styles.subtitle, { marginTop: 6 }]}>Signed in: {currentUser.email}</ThemedText>
            )}
          </View>
        </View>
      </LinearGradient>
          ...
          <View style={styles.contentContainer}>
            ...
            >
              <View style={styles.sectionHeader}>
                ...
                <View style={styles.sectionHeaderText}>
                  <ThemedText type="title" style={styles.sectionTitle}>Customer Details</ThemedText>
                  <ThemedText style={styles.sectionSubtitle}>Enter customer information</ThemedText>
                </View>
              </View>
              ...
              <View style={{ position: 'relative' }}>
                <TextInput
                  ...
                  onFocus={() => customer.name && setShowCustomerSuggestions(true)}
                  autoCorrect={false}
                ...
                {showCustomerSuggestions && filteredCustomerSuggestions.length > 0 && (
                  <View style={styles.suggestionBox}>
                    {filteredCustomerSuggestions.map(suggestion => (
                      <TouchableOpacity key={suggestion.id} style={styles.suggestionItem} onPress={() => handleCustomerSuggestionSelect(suggestion)}>
            ...
            >
              <View style={styles.sectionHeader}>
                ...
                <View style={styles.sectionHeaderText}>
                  <ThemedText type="title" style={styles.sectionTitle}>Sale Items</ThemedText>
              ...
              <View style={styles.paymentInputs}>
                <View style={styles.paymentInputContainer}>
                  ...
                  <TextInput
                    ...
                    placeholder="0"
                    value={cashReceived ? Number(cashReceived.replace(/,/g, '')).toLocaleString() : ''}
                    onChangeText={val => {
                      const raw = val.replace(/,/g, '');
                      const cashValue = Number(raw) || 0;
                      const maxCash = customerPrevBalance + totalAmount;
                      if (cashValue > maxCash) {
                        setErrors((prev: any) => ({ ...prev, cashReceived: `Cash cannot exceed Rs ${maxCash.toLocaleString()}` }));
                        return;
                      } else {
                        setErrors((prev: any) => ({ ...prev, cashReceived: undefined }));
                        setCashReceived(raw);
                      }
                    }}
                    ...
                    placeholderTextColor="#94a3b8"
                  />
                  {errors.cashReceived && <Text style={styles.errorText}>{errors.cashReceived}</Text>}
                </View>
                ...
                <View style={styles.paymentInputContainer}>
                  ...
                  <TextInput
                    ...
                    placeholder="0"
                    value={(customerPrevBalance + totalAmount - cash).toLocaleString()}
                    editable={false}
                    keyboardType="numeric"
                    placeholderTextColor="#94a3b8"
                  />
                  {(customerPrevBalance !== 0 || totalAmount !== 0 || cash !== 0) && (
                    <Text style={styles.calculationText}>
                      Previous: Rs {customerPrevBalance.toLocaleString()} + Sale: Rs {totalAmount.toLocaleString()} - Cash: Rs {cash.toLocaleString()}
                    </Text>
                  )}
                </View>
              </View>
            ...
            >
              <View style={styles.summaryHeader}>
                ...
                <View style={styles.summaryHeaderText}>
                  <ThemedText type="title" style={styles.summaryTitle}>Sale Summary</ThemedText>
              ...
              <View style={styles.statsRow}>
                ...
                <View style={[styles.statCard, styles.cashCard]}>
                  ...
                  <ThemedText style={styles.statLabel}>Cash Received</ThemedText>
                </View>
                
                <View style={[styles.statCard, styles.accountCard]}>
                  <MaterialIcons name="credit-card" size={20} color="#8b5cf6" />
                  <ThemedText style={styles.statNumber}>Rs {(customerPrevBalance + totalAmount - cash).toLocaleString()}</ThemedText>
            ...
            <View style={styles.actionButtons}>
              ...
              >
                ...
                <ThemedText style={styles.historyBtnText}>View Sales History</ThemedText>
              </Pressable>
            </View>
          </View>
        </ScrollView>
                  ...
                  {filteredItemSuggestions.map(suggestion => (
                    <TouchableOpacity key={suggestion.id} style={styles.suggestionItem} onPress={() => handleItemSuggestionSelect(suggestion)}>
                      <View style={styles.suggestionContent}>
                        ...
                        <View style={styles.suggestionText}>
                          <Text style={styles.suggestionName}>{suggestion.name}</Text>
                          <Text style={styles.suggestionBalance}>Price: Rs {Number(suggestion.sellingPrice || 0).toLocaleString()}</Text>
                        </View>
                      </View>
                    </TouchableOpacity>
                  ))}
