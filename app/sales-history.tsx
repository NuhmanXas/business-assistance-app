...
export default function SalesHistoryScreen() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q as any, (snapshot: QuerySnapshot<DocumentData>) => {
      setSalesHistory(snapshot.docs.map((d: QueryDocumentSnapshot<DocumentData>) => ({ id: d.id, ...(d.data() as any) })));
    });
    return () => unsub();
  }, [currentUser]);
  ...
  const filtered = applyFilters(salesHistory);
  ...
  return (
    <ThemedView style={styles.container}>
      <SalesDetailModal sale={selectedSale} visible={modalVisible} onClose={closeModal} />
      ...
      >
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerTitles}>
            <ThemedText type="title" style={styles.title}>Sales History</ThemedText>
            <ThemedText type="subtitle" style={styles.subtitle}>View and manage your sales records</ThemedText>
          </View>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.searchSection}>
            <View style={styles.searchInputContainer}>
              ...
              <TextInput
                placeholder="Quick search customer or item"
                value={searchQuery}
                  ...
                  <View style={styles.filterInputGroup}>
                    ...
                    {customerFilter.trim().length > 0 && (
                      <View style={styles.suggestionsBox}>
                        {Array.from(new Set(salesHistory.map(p => (p.customerName || '').toString()))).filter(v => v && v.toLowerCase().includes(customerFilter.toLowerCase())).slice(0, 6).map((v, idx) => (
                          <TouchableOpacity key={idx} onPress={() => setCustomerFilter(v)} style={styles.suggestionItem}>
                            <Text>{v}</Text>
                          </TouchableOpacity>
                        ))}
                      </View>
          ...
          <View style={styles.salesListSection}>
            <View style={styles.sectionHeader}>
              <View style={styles.sectionHeaderLeft}>
                <MaterialIcons name="receipt-long" size={20} color="#667eea" />
                <ThemedText type="title" style={styles.sectionTitle}>Sales Records</ThemedText>
              </View>
            </View>
            ...
            {filtered.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                {salesHistory.length === 0 ? (
                  <>
                    <ThemedText style={styles.emptyText}>No sales records yet</ThemedText>
                    <ThemedText style={styles.emptySubtext}>Start making sales to see your transaction history here</ThemedText>
                  </>
                ) : (
                  <>
                    <ThemedText style={styles.emptyText}>No sales match your filters</ThemedText>
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  const initials = ((item.customerName || '').split(' ').map((s: any) => s?.[0] || '').join('').slice(0, 2)).toUpperCase();
                  return (
                    <TouchableOpacity activeOpacity={0.85} onPress={() => openSale(item)}>
                          ...
                          <View style={styles.amountSection}>
                            ...
                            <View style={styles.paymentBreakdown}>
                              <View style={styles.paymentRow}>
                                ...
                                <Text style={styles.cashAmount}>Rs {item.cashReceived ?? 0}</Text>
                              </View>
                              <View style={styles.paymentRow}>
...
function SalesDetailModal({ sale, visible, onClose }: { sale: any, visible: boolean, onClose: () => void }) {
  // Hooks must be called unconditionally at the top level of the component.
  const viewRef = useRef<any>(null);
  const [sharing, setSharing] = useState(false);

  if (!sale) return <Modal visible={visible} transparent animationType="slide"><View style={{ flex: 1 }} /></Modal>;

  const d = (sale?.date && (sale.date?.toDate ? sale.date.toDate() : new Date(sale.date))) || new Date();
...
const saveWithExpo = async (uri: string, filename: string, platform: 'ios' | 'android') => {
 ...
 const handleDownload = async () => {
   try {
     ...
     const filename = `sale_${sale.invoiceNumber || sale.id || Date.now()}.png`;
         ...
         <View ref={viewRef} collapsable={false} style={{ backgroundColor: '#fff' }}>
           <ScrollView contentContainerStyle={{ padding: 18, backgroundColor: '#fff' }}>
             <View style={styles.docHeader}>
               <Text style={styles.docTitle}>Sales Document</Text>
               <Text style={styles.docSub}>Invoice / Sales Receipt</Text>
             </View>

             <View style={styles.docMeta}>
               <View style={styles.metaColumn}>
                 <Text style={styles.metaLabel}>Invoice / Ref:</Text>
                 <Text style={styles.metaValue}>{sale.invoiceNumber || sale.reference || sale.id}</Text>
               ...
               <View style={styles.metaColumn}>
                 ...
                 {sale.customerContact && <Text style={styles.metaValue}>{sale.customerContact}</Text>}
                 {sale.customerAddress && <Text style={styles.metaValue}>{sale.customerAddress}</Text>}
               </View>
             </View>
             ...
             <View style={styles.totals}>
               ...
               {sale.taxAmount != null && (
                 <View style={styles.totalsRow}>
                   ...
                   <Text style={styles.totalsValue}>Rs {Number(sale.taxAmount).toLocaleString()}</Text>
                 </View>
               )}
               <View style={styles.totalsRow}>
               ...
               <View style={styles.totalsRow}>
                 <Text style={styles.totalsLabel}>Cash Received</Text>
                 <Text style={styles.totalsValue}>Rs {Number(sale.cashReceived ?? 0).toLocaleString()}</Text>
               </View>
               <View style={styles.totalsRow}>
                 <Text style={styles.totalsLabel}>On Account</Text>
                 <Text style={styles.totalsValue}>Rs {Number(sale.onAccount ?? 0).toLocaleString()}</Text>
               </View>
             </View>
             ...
             {sale.notes && (
               <View style={{ marginTop: 12 }}>
                 ...
                 <Text style={styles.metaValue}>{sale.notes}</Text>
               </View>
             )}
           </ScrollView>
         </View>
